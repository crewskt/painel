class BinanceDatafeed {
  ...
  async binanceServerTime() {
    try {
      const response = await fetch(`${this.binanceHost}/fapi/v1/time`);
  ...
  async binanceSymbols() {
    try {
      const response = await fetch(`${this.binanceHost}/fapi/v1/exchangeInfo`);
      const json = await response.json();
      if (this.debug) {
        console.log(json);
      }
  ...
  async binanceKlines(symbol, interval, startTime, endTime, limit) {
    const url = `${this.binanceHost}/fapi/v1/klines?symbol=${symbol}&interval=${interval}&limit=${limit}&startTime=${startTime}&endTime=${endTime}`;
    try {
